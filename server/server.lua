...
AddEventHandler('xakra_steal:ReloadInventory', function(steal_source, player_source)
    ...
    TriggerEvent('vorpCore:getUserInventory', tonumber(steal_source), function(getInventory)
    ...
    TriggerEvent('vorpCore:getUserWeapons', tonumber(steal_source), function(getUserWeapons)
...
RegisterServerEvent('xakra_steal:MoveTosteal2')
AddEventHandler('xakra_steal:MoveTosteal2', function(obj, steal_source)
    local _source = source

    local decode_obj = json.decode(obj)

    if decode_obj.type == 'item_standard' then
        local canCarry = VorpInv.canCarryItem(steal_source, decode_obj.item.name, decode_obj.number)
        if canCarry then
            VorpInv.subItem(_source, decode_obj.item.name, decode_obj.number)
            VorpInv.addItem(steal_source, decode_obj.item.name, decode_obj.number)
    ...
    elseif decode_obj.type == 'item_weapon' then
        VorpInv.canCarryWeapons(steal_source, 1, function(cb)
            local canCarry = cb
            if canCarry then
                VorpInv.subWeapon(_source, decode_obj.item.id)
                VorpInv.giveWeapon(steal_source, decode_obj.item.id, 0)
                TriggerEvent('xakra_steal:ReloadInventory', steal_source, _source)
            else
...
RegisterServerEvent('xakra_steal:TakeFromsteal2')
AddEventHandler('xakra_steal:TakeFromsteal2', function(obj, steal_source)
    local _source = source
    ...
    if decode_obj.type == 'item_standard' then
        ...
        if canCarry then
            VorpInv.subItem(steal_source, decode_obj.item.name, decode_obj.number)
    ...
    elseif decode_obj.type == 'item_weapon' then
        VorpInv.canCarryWeapons(_source, 1, function(cb)
            local canCarry = cb
            if canCarry then
                VorpInv.subWeapon(steal_source, decode_obj.item.id)
